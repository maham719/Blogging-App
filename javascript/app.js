...
const getCurrentUser = async (uid) => {
  ...
  if (docSnap.exists()) {
    ...
    if (location.pathname.endsWith("/blog.html")) {
      // Navbar name (likely <a> or <span>)
      if (fullName) fullName.innerHTML = docSnap.data().name;

    } else if (
      !location.pathname.endsWith("/allblog.html") &&
      !location.pathname.endsWith("/user.html")
    ) {
      // Profile page (likely inputs)
      if (fullName) fullName.value = docSnap.data().name;
      if (email) email.value = docSnap.data().email;
      if (userUid) userUid.value = docSnap.id;
      if (profilePic && docSnap.data().profile) {
        profilePic.src = docSnap.data().profile;
      }
    }
...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    if (location.pathname.endsWith("/allblog.html")) {
      getAllBlogs();
    }

    // Redirect only if we're not already on an allowed page
    if (
      !(
        location.pathname.endsWith("/user.html") ||
        location.pathname.endsWith("/allblog.html") ||
    ...
    if (!location.pathname.endsWith("/signUpIn.html")) {
